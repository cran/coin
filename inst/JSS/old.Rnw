...
Before dealing with the computational details of \pkg{coin}, consider the following motivating example.
\cite{Bergmann:2000} compared test statistics and $p$ values of the Wilcoxon-Mann-Whitney
rank sum test as reported by eleven statistical packages and observed practically 
relevant numerical differences for a 2-sample problem with twelve observations in
each group. More specifically, $n = 24$ rats received a fixed oral dose of a centrally 
acting muscle relaxant as active treatment or a saline solvent as control. The animals were 
placed on a rotating cylinder and the length of time each rat remains on the cylinder is
measured, up to a maximum of $300$ seconds. The rats were randomly assigned to the control 
and treatment group, thus a re-randomization test as implemented 
in \Rcmd{independence_test} is the appropriate way to investigate
if the response is independent of the group assignment. A rather general 
theoretical framework
for such re-randomization tests and a class structure for representing 
its ingredients are described in Section~\ref{sec:theory}.

%%TH: erstmal Beispiel zeigen und dann lowlevel immer mitlaufen lassen.
%% + user defined trafo
%% aber erst framework kurz

In our example, the hypothesis of independence of length of time and group assigment is
specified by a formula which, together with a data frame \code{rotarod}, 
serve are arguments to \Rcmd{independence_test}:
<<motivation-perm, echo = TRUE>>=
library("coin")
independence_test(time ~ group, data = rotarod)
@
Even beginners in \proglang{R} can easily interpret this output since
it is represented in exactly the same way as the output
of classical tests in \pkg{stats}. More details on the user interface are 
given in Section~\ref{sec:ui}.
...
average ranks. The \code{ytrans} argument takes a function, such as \Rcmd{rank} in 
our case:
...
<<new, echo = TRUE>>=
it <- new("IndependenceProblem", y = rotarod["time"], x = rotarod["group"])
itp <- new("IndependenceTestProblem", it)
its <- new("IndependenceTestStatistic", itp)
sit <- new("ScalarIndependenceTestStatistic", its, alternative = "two.sided")
new("ScalarIndependenceTest", statistic = sit, 
    distribution = AsymptNullDistribution(sit))

new("ScalarIndependenceTest", statistic = sit,
    distribution = ExactNullDistribution(sit, algorithm = "shift"))

itp <- new("IndependenceTestProblem", it, ytrafo = rank)
its <- new("IndependenceTestStatistic", itp)
sit <- new("ScalarIndependenceTestStatistic", its, alternative = "two.sided")
new("ScalarIndependenceTest", statistic = sit, 
    distribution = ExactNullDistribution(sit, algorithm = "shift"))
...
\section{A class structure for permutation tests} \label{sec:theory}
...
\R^{pq}$ onto the real line can be of arbitrary form. 
...
%%TH: distribution: neue algorithmen (network z.B.) erklaeren
...
distribution. Within each block, a sufficiently large number of
...
distribution of these test statistics is an approximate to the 
...
\citet[Theorem 2.3]{StrasserWeber1999} proved that the   
...
%% TH: dort erklaeren, wo gebraucht
\section{Generic functions} \label{sec:methods}
...
an object of class \Rclass{NullDistribution} can be specified as \code{distribution}
...
\end{tabular}
%%TH: Better caption
\caption{Specifications of the conditional counterparts of 
...
%Z% mention required functionality somewhere, such as modeltools and mvtnorm
...
%Z% also mention required packages and their version.
