...
(dp_uas <- dperm(it_uas, 0:1))
stopifnot(isequal(dp_uas, c(dperm(it_uas, 0), dperm(it_uas, 1))))
(pp_uas <- pperm(it_uas, 0:1))
stopifnot(isequal(pp_uas, c(pperm(it_uas, 0), pperm(it_uas, 1))))
(qp_uas <- qperm(it_uas, c(0.5, 0.75)))
stopifnot(isequal(qp_uas, c(qperm(it_uas, 0.5), qperm(it_uas, 0.75))))
(pv_uas <- pvalue(it_uas@distribution, 0:1))
stopifnot(isequal(pv_uas, c(pvalue(it_uas@distribution, 0),
                            pvalue(it_uas@distribution, 1))))

### univariate, asymptotic and quad
it_uaq <- independence_test(y1 ~ x,  data = dta,
                            distribution = "asymptotic", teststat = "quad")
(dp_uaq <- dperm(it_uaq, 0:1))
stopifnot(isequal(dp_uaq, c(dperm(it_uaq, 0), dperm(it_uaq, 1))))
(pp_uaq <- pperm(it_uaq, 0:1))
stopifnot(isequal(pp_uaq, c(pperm(it_uaq, 0), pperm(it_uaq, 1))))
(qp_uaq <- qperm(it_uaq, c(0.5, 0.75)))
stopifnot(isequal(qp_uaq, c(qperm(it_uaq, 0.5), qperm(it_uaq, 0.75))))
(pv_uaq <- pvalue(it_uaq@distribution, 0:1))
stopifnot(isequal(pv_uaq, c(pvalue(it_uaq@distribution, 0),
                            pvalue(it_uaq@distribution, 1))))

### multivariate, asymptotic and max
it_mam <- independence_test(y1 + y2 ~ x,  data = dta,
                            distribution = "asymptotic", teststat = "max")
(dp_mam <- dperm(it_mam, 0:1))
## stopifnot(isequal(dp_mam, c(dperm(it_mam, 0), dperm(it_mam, 1))))
(pp_mam <- pperm(it_mam, 0:1))                               # failed in < 1.1-0
stopifnot(isequal(pp_mam, c(pperm(it_mam, 0), pperm(it_mam, 1))))
(qp_mam <- qperm(it_mam, c(0.5, 0.75)))                      # failed in < 1.1-0
stopifnot(isequal(qp_mam, c(qperm(it_mam, 0.5), qperm(it_mam, 0.75))))
(pv_mam <- pvalue(it_mam@distribution, 0:1))
stopifnot(isequal(pv_mam, c(pvalue(it_mam@distribution, 0),
                            pvalue(it_mam@distribution, 1))))

### multivariate, asymptotic and quad
it_maq <- independence_test(y1 + y2 ~ x,  data = dta,
                            distribution = "asymptotic", teststat = "quad")
(dp_maq <- dperm(it_maq, 0:1))
stopifnot(isequal(dp_maq, c(dperm(it_maq, 0), dperm(it_maq, 1))))
(pp_maq <- pperm(it_maq, 0:1))
stopifnot(isequal(pp_maq, c(pperm(it_maq, 0), pperm(it_maq, 1))))
(qp_maq <- qperm(it_maq, c(0.5, 0.75)))
stopifnot(isequal(qp_maq, c(qperm(it_maq, 0.5), qperm(it_maq, 0.75))))
(pv_maq <- pvalue(it_maq@distribution, 0:1))
stopifnot(isequal(pv_maq, c(pvalue(it_maq@distribution, 0),
                            pvalue(it_maq@distribution, 1))))

### univariate, approximate and scalar
it_ums <- independence_test(y1 ~ x,  data = dta,
                            distribution = "approximate", teststat = "scalar")
(dp_ums <- dperm(it_ums, 0:1))
stopifnot(isequal(dp_ums, c(dperm(it_ums, 0), dperm(it_ums, 1))))
(pp_ums <- pperm(it_ums, 0:1))
stopifnot(isequal(pp_ums, c(pperm(it_ums, 0), pperm(it_ums, 1))))
(qp_ums <- qperm(it_ums, c(0.5, 0.75)))
stopifnot(isequal(qp_ums, c(qperm(it_ums, 0.5), qperm(it_ums, 0.75))))
(pv_ums <- pvalue(it_ums@distribution, 0:1))
stopifnot(isequal(pv_ums, c(pvalue(it_ums@distribution, 0),
                            pvalue(it_ums@distribution, 1))))
(mp_ums <- midpvalue(it_ums@distribution, 0:1))
stopifnot(isequal(mp_ums, c(midpvalue(it_ums@distribution, 0),
                            midpvalue(it_ums@distribution, 1))))
(pi_ums <- pvalue_interval(it_ums@distribution, 0:1))
stopifnot(isequal(pi_ums, cbind(pvalue_interval(it_ums@distribution, 0),
                                pvalue_interval(it_ums@distribution, 1))))

### univariate, approximate and quad
it_umq <- independence_test(y1 ~ x,  data = dta,
                            distribution = "approximate", teststat = "quad")
(dp_umq <- dperm(it_umq, 0:1))
stopifnot(isequal(dp_umq, c(dperm(it_umq, 0), dperm(it_umq, 1))))
(pp_umq <- pperm(it_umq, 0:1))
stopifnot(isequal(pp_umq, c(pperm(it_umq, 0), pperm(it_umq, 1))))
(qp_umq <- qperm(it_umq, c(0.5, 0.75)))
stopifnot(isequal(qp_umq, c(qperm(it_umq, 0.5), qperm(it_umq, 0.75))))
(pv_umq <- pvalue(it_umq@distribution, 0:1))
stopifnot(isequal(pv_umq, c(pvalue(it_umq@distribution, 0),
                            pvalue(it_umq@distribution, 1))))
(mp_umq <- midpvalue(it_umq@distribution, 0:1))
stopifnot(isequal(mp_umq, c(midpvalue(it_umq@distribution, 0),
                            midpvalue(it_umq@distribution, 1))))
(pi_umq <- pvalue_interval(it_umq@distribution, 0:1))
stopifnot(isequal(pi_umq, cbind(pvalue_interval(it_umq@distribution, 0),
                                pvalue_interval(it_umq@distribution, 1))))

### multivariate, approximate and max
it_mmm <- independence_test(y1 + y2 ~ x,  data = dta,
                            distribution = "approximate", teststat = "max")
(dp_mmm <- dperm(it_mmm, 0:1))
stopifnot(isequal(dp_mmm, c(dperm(it_mmm, 0), dperm(it_mmm, 1))))
(pp_mmm <- pperm(it_mmm, 0:1))
stopifnot(isequal(pp_mmm, c(pperm(it_mmm, 0), pperm(it_mmm, 1))))
(qp_mmm <- qperm(it_mmm, c(0.5, 0.75)))
stopifnot(isequal(qp_mmm, c(qperm(it_mmm, 0.5), qperm(it_mmm, 0.75))))
(pv_mmm <- pvalue(it_mmm@distribution, 0:1))
stopifnot(isequal(pv_mmm, c(pvalue(it_mmm@distribution, 0),
                            pvalue(it_mmm@distribution, 1))))
(mp_mmm <- midpvalue(it_mmm@distribution, 0:1))
stopifnot(isequal(mp_mmm, c(midpvalue(it_mmm@distribution, 0),
                            midpvalue(it_mmm@distribution, 1))))
(pi_mmm <- pvalue_interval(it_mmm@distribution, 0:1))
stopifnot(isequal(pi_mmm, cbind(pvalue_interval(it_mmm@distribution, 0),
                                pvalue_interval(it_mmm@distribution, 1))))

### multivariate, approximate and quad
it_mmq <- independence_test(y1 + y2 ~ x,  data = dta,
                            distribution = "approximate", teststat = "quad")
(dp_mmq <- dperm(it_mmq, 0:1))
stopifnot(isequal(dp_mmq, c(dperm(it_mmq, 0), dperm(it_mmq, 1))))
(pp_mmq <- pperm(it_mmq, 0:1))
stopifnot(isequal(pp_mmq, c(pperm(it_mmq, 0), pperm(it_mmq, 1))))
(qp_mmq <- qperm(it_mmq, c(0.5, 0.75)))
stopifnot(isequal(qp_mmq, c(qperm(it_mmq, 0.5), qperm(it_mmq, 0.75))))
(pv_mmq <- pvalue(it_mmq@distribution, 0:1))
stopifnot(isequal(pv_mmq, c(pvalue(it_mmq@distribution, 0),
                            pvalue(it_mmq@distribution, 1))))
(mp_mmq <- midpvalue(it_mmq@distribution, 0:1))
stopifnot(isequal(mp_mmq, c(midpvalue(it_mmq@distribution, 0),
                            midpvalue(it_mmq@distribution, 1))))
(pi_mmq <- pvalue_interval(it_mmq@distribution, 0:1))
stopifnot(isequal(pi_mmq, cbind(pvalue_interval(it_mmq@distribution, 0),
                                pvalue_interval(it_mmq@distribution, 1))))

### univariate, exact and scalar
it_ues <- independence_test(y1 ~ x,  data = dta,
                            distribution = "exact", teststat = "scalar")
(dp_ues <- dperm(it_ues, 0:1))
stopifnot(isequal(dp_ues, c(dperm(it_ues, 0), dperm(it_ues, 1))))
(pp_ues <- pperm(it_ues, 0:1))
stopifnot(isequal(pp_ues, c(pperm(it_ues, 0), pperm(it_ues, 1))))
(qp_ues <- qperm(it_ues, c(0.5, 0.75)))
stopifnot(isequal(qp_ues, c(qperm(it_ues, 0.5), qperm(it_ues, 0.75))))
(pv_ues <- pvalue(it_ues@distribution, 0:1))
stopifnot(isequal(pv_ues, c(pvalue(it_ues@distribution, 0),
                            pvalue(it_ues@distribution, 1))))
(mp_ues <- midpvalue(it_ues@distribution, 0:1))
stopifnot(isequal(mp_ues, c(midpvalue(it_ues@distribution, 0),
                            midpvalue(it_ues@distribution, 1))))
(pi_ues <- pvalue_interval(it_ues@distribution, 0:1))
stopifnot(isequal(pi_ues, cbind(pvalue_interval(it_ues@distribution, 0),
                                pvalue_interval(it_ues@distribution, 1))))

### univariate, exact and quad
it_ueq <- independence_test(y1 ~ x,  data = dta,
                            distribution = "exact", teststat = "quad")
(dp_ueq <- dperm(it_ueq, 0:1))
stopifnot(isequal(dp_ueq, c(dperm(it_ueq, 0), dperm(it_ueq, 1))))
(pp_ueq <- pperm(it_ueq, 0:1))
stopifnot(isequal(pp_ueq, c(pperm(it_ueq, 0), pperm(it_ueq, 1))))
(qp_ueq <- qperm(it_ueq, c(0.5, 0.75)))
stopifnot(isequal(qp_ueq, c(qperm(it_ueq, 0.5), qperm(it_ueq, 0.75))))
(pv_ueq <- pvalue(it_ueq@distribution, 0:1))
stopifnot(isequal(pv_ueq, c(pvalue(it_ueq@distribution, 0),
                            pvalue(it_ueq@distribution, 1))))
(mp_ueq <- midpvalue(it_ueq@distribution, 0:1))
stopifnot(isequal(mp_ueq, c(midpvalue(it_ueq@distribution, 0),
                            midpvalue(it_ueq@distribution, 1))))
(pi_ueq <- pvalue_interval(it_ueq@distribution, 0:1))
stopifnot(isequal(pi_ueq, cbind(pvalue_interval(it_ueq@distribution, 0),
                                pvalue_interval(it_ueq@distribution, 1))))
